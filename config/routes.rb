MotorCarExport::Application.routes.draw do
	constraints(host: /^www.*/) do
		root to: redirect { |params, request| "#{request.protocol}#{request.host.gsub('www.', '')}" }
		get '*url', to: redirect { |params, request| "#{request.protocol}#{request.host.gsub('www.', '')}#{params[:url]}" }
	end
